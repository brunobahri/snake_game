...
void init_snake(Snake *snake) {
    ...
    snake->body[0].x = 10;
    snake->body[0].y = 10;
    snake->body[0].dir = 'r';  // Direção inicial da cabeça
    snake->direction = 'r';
}
...
void move_snake(Snake *snake) {
    for (int i = snake->length - 1; i > 0; i--) {
        ...
        snake->body[i].dir = snake->body[i - 1].dir;  // Atualiza a direção
    }
...
void grow_snake(Snake *snake) {
    ...
    snake->body[snake->length] = snake->body[snake->length - 1];  // Copia o último segmento
    snake->length++;
}
